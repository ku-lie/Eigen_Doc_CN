...
![](http://latex.codecogs.com/svg.latex?a^T)，共轭![](http://latex.codecogs.com/svg.latex?\bar{a})，和伴随（即共轭转置）![](http://latex.codecogs.com/svg.latex?a^*)分别由成员函数transpose()、conjugate(), 和adjoint()获得。
...
**注意：**对于担心性能的BLAS用户，可以使用*c.noalias()-=2\*a.adjoint()\*b*；完全优化并触发单个类似gemm的函数调用。
